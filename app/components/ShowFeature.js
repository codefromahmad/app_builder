...
const ShowFeature = ({
  ...
  const checkIfCustom =
    selectedFeature &&
    typeof selectedFeature.id === "string" &&
    selectedFeature.id.startsWith("custom_");
  ...
  return (
    ...
    >
      {platform === "mobile" ? (
        <div
          className={`border-2 border-[#A6A6A6] p-2 ${
            !checkIfCustom
              ? "w-48"
              : "w-48 h-96 flex items-center justify-center"
          } rounded-xl`}
        >
        ...
        <div
          className={`border-2 border-[#A6A6A6] ${
            checkIfCustom && "flex items-center justify-center"
          } p-2 w-[500px] h-[300px] rounded-xl`}
        >
          <Image
            width={1024}
            height={100}
            ...
            className={`object-fill ${
              !checkIfCustom ? "w-full h-full" : "w-40 h-40"
            } rounded-xl`}
          />
        </div>
      )}
        ...
        <div className="w-1/3">
          <div className="flex items-center gap-2">
            ...
            {!checkIfCustom ? (
              ...
              <div
                onClick={() => handleCustomFeaturesSelection(selectedFeature)}
                className="bg-white hover:bg-slate-50 duration-300 w-7 h-7 rounded-md items-center justify-center flex border-[1px] cursor-pointer"
              >
                <MdDeleteOutline className="text-black" />
              </div>
